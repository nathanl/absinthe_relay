defmodule Absinthe.Relay.KeysetConnectionTest do
  use Absinthe.Relay.Case, async: true
  # import ExUnit.CaptureLog

  alias Absinthe.Relay.KeysetConnection

  if Code.ensure_loaded?(Ecto) do

    def keyset_cursor(id) do
      Base.encode64("arrayconnection:#{id}")
    end
  ...
  # before: 100, first: 10 = id < 100 ORDER BY id ASC LIMIT 10
    ...
    describe "keyset_params_for" do
      test ":after + :first" do
        p = KeysetConnection.keyset_params_from(%{after: 100, first: 10})
    ...
    #           Absinthe.Relay.KeysetConnection.from_list(
    ...
    #       resolve fn _, args -> Absinthe.Relay.KeysetConnection.from_list(@people, args) end
    #     end
    #
    ...
    #     capture_log(fn ->
    #       {:ok, %{edges: [%{node: node} | _]}} = KeysetConnection.from_list([record], %{first: 1})
    ...
    #              KeysetConnection.from_list([record], %{first: 1})
    ...
    #          }
    #        }} = KeysetConnection.from_list(records, %{first: 2})
    #     )
    #   end
    # end
    #
    ...
    #     KeysetConnection.from_slice([%{foo: :bar}], nil)
    #   end
    # end
    #
    # describe ".offset_and_limit_for_query/2" do
    #   test "with a cursor" do
    #     assert KeysetConnection.offset_and_limit_for_query(%{first: 10, before: @offset_cursor_1}, []) ==
    #              {:ok, 1, 10}
    #
    #     assert KeysetConnection.offset_and_limit_for_query(%{first: 5, before: @offset_cursor_2}, []) ==
    #              {:ok, 5, 5}
    #
    #     assert KeysetConnection.offset_and_limit_for_query(%{last: 10, before: @offset_cursor_1}, []) ==
    #              {:ok, 0, 1}
    #
    #     assert KeysetConnection.offset_and_limit_for_query(%{last: 5, before: @offset_cursor_2}, []) ==
    #              {:ok, 0, 5}
    #
    #     # Should be the same result even if nil :first and :after are provided:
    #     assert KeysetConnection.offset_and_limit_for_query(
    #              %{first: nil, after: nil, last: 5, before: @offset_cursor_2},
    #              []
    #            ) ==
    #              {:ok, 0, 5}
    #
    #     assert KeysetConnection.offset_and_limit_for_query(%{last: 3, before: @offset_cursor_2}, []) ==
    #              {:ok, 2, 3}
    #   end
    #
    #   test "without a cursor" do
    #     assert KeysetConnection.offset_and_limit_for_query(%{first: 10, before: nil}, []) == {:ok, 0, 10}
    #     assert KeysetConnection.offset_and_limit_for_query(%{first: 5, after: nil}, []) == {:ok, 0, 5}
    #
    #     assert KeysetConnection.offset_and_limit_for_query(%{last: 10, before: nil}, count: 30) ==
    #              {:ok, 20, 10}
    #
    #     assert KeysetConnection.offset_and_limit_for_query(%{last: 5, after: nil}, count: 30) ==
    #              {:ok, 25, 5}
    #   end
    #
    #   test "with an invalid cursor" do
    #     assert KeysetConnection.offset_and_limit_for_query(%{first: 10, before: @invalid_cursor_1}, []) ==
    #              {:error, "Invalid cursor provided as `before` argument"}
    #
    #     assert KeysetConnection.offset_and_limit_for_query(%{first: 10, before: @invalid_cursor_2}, []) ==
    #              {:error, "Invalid cursor provided as `before` argument"}
    #
    #     assert KeysetConnection.offset_and_limit_for_query(%{first: 10, before: @invalid_cursor_3}, []) ==
    #              {:error, "Invalid cursor provided as `before` argument"}
    #
    #     assert KeysetConnection.offset_and_limit_for_query(
